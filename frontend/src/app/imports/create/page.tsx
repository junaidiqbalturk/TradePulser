...
export default function CreateImportOrderPage() {
    ...
    useEffect(() => {
        ...
        ]).then(([clientsRes, poRes]) => {
            setClients(clientsRes.data);
            setPurchaseOrders(poRes.data.filter((po: any) => ['approved', 'ordered'].includes(po.status)));
        }).catch(console.error);
    }, []);
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        setErrorMsg("");
        try {
            await api.post("/import-orders", values);
            window.location.href = "/imports";
        } catch (error: any) {
            setErrorMsg(error.response?.data?.message || "Failed to create import order");
        }
    }
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-8 pb-12">
            <div className="space-y-8">
                <div className="mb-6 flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Create Import Order</h1>
                    </div>
                </div>
                ...
                <Card className="border border-border shadow-sm overflow-hidden rounded-xl bg-card">
                    <CardHeader className="bg-muted/40 border-b border-border">
                        ...
                        <CardDescription className="text-muted-foreground mt-1 text-sm">Log a new incoming shipment for a client.</CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <FormField control={form.control} name="origin" render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl><Input placeholder="e.g. Shanghai, China" {...field} /></FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )} />

                                        <FormField control={form.control} name="expected_arrival" render={({ field }) => (
                                            <FormItem>
                                        ...
                                        <FormField control={form.control} name="purchase_order_id" render={({ field }) => (
                                            <FormItem>
                                    ...
                                    <div className="pt-2">
                                        <FormField control={form.control} name="notes" render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl><Textarea placeholder="Additional shipment details, vessel name, container marks..." className="min-h-[100px]" {...field} /></FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )} />
                                    </div>
